...
This is the machine-learning way to make the simulated rover follow a line, instead of the
hand-tuned math in the [PID baseline](pid-line-follower.md). PPO (Proximal Policy Optimization) is
a reinforcement-learning algorithm: instead of a human picking control gains, a neural network
policy learns them by trial and error, driving the same CAD rover on the same 2-inch circle,
figure-eight, and oval tracks under the same success criteria. Domain randomization and BAM motor
dynamics are covered separately in the
[integration and evaluation guide](dr-bam-line-follower.md), including parameter ranges and the
physical deployment interface.

The nominal policy trained here completed **40/40 independent test episodes on each of the three
tracks**, including figure-eight — see [Step 3](#step-3-read-the-results) for the checkpoint,
metrics, and artifacts.

## Before you start

You should already have the workspace set up (see [workspace-setup.md](workspace-setup.md)). All
commands below are run from inside `rover_mujoco/`. Training a policy from scratch takes a while;
if you'd rather skip straight to using an already-trained one, see
[Download and run saved policies](policy-downloads.md) and come back here later.

## Step 1: Train a policy

This kicks off PPO training across all three tracks in parallel simulated environments, and logs
progress to Weights & Biases as it goes:
...
A few common variations on that same command:
...
## Step 2: Watch it, evaluate it, and plot its training curves

Once you have a checkpoint (your own, or a downloaded one), first just watch it drive:
...
Then run a larger, headless batch of test episodes to actually measure how good it is, and export
the training curves TensorBoard/W&B recorded:
...
If you want to script against the environment directly instead of going through those two scripts,
it's a normal registered Gymnasium environment:
...
The first plot is a training curve — reward per rollout over time. A policy that's learning
successfully shows reward climbing and then leveling off as it converges; a flat line from the
start, or one that never stops climbing, would suggest it hasn't finished learning yet:

![Nominal PPO training curves, measured in additional environment steps](images/Resources/ppo_nominal_training.png)

The second plot overlays every test episode's actual path (one line per episode) against the
track. A policy that's generalized well keeps all 60 lines tightly bunched and close to the track
shape across every seed, not just a few lucky ones:
...
The sections below explain exactly how the environment, reward, and training are built — useful
if you're extending this baseline or debugging unexpected behavior, not required to just run it.
